...
- For public classes, protocols, DTOs, contracts, use cases, adapters, and important public methods, use standard Python docstrings (`"""..."""`) when the explanation helps IDE hover hints and code review.
- Good docstrings should describe the object or method responsibility, workflow state, boundary, invariant, or important constraint. Prefer useful domain/process meaning such as "an item extracted according to an Extraction Task but not yet stored" over labels such as "HTTP DTO", "adapter", "schema", or translations of the class name.
- When writing docstrings on Pydantic models, treat the text as user-facing schema description because it can appear in JSON Schema, OpenAPI, or LLM structured-output metadata. Keep it precise, stable, and meaningful outside the code editor.
- Keep docstrings concise. One sentence is enough when the responsibility is simple; use a short second paragraph only to clarify lifecycle state, persistence context, evidence requirements, or cross-boundary behavior.
